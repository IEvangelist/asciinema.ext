...
- **`scripts/demo/`** — a Playwright-driven HTML mockup of VS Code that the recorder walks through scene-by-scene. Run `npm run record:demo` to regenerate `media/demo-explorer.webm` deterministically (no GitHub auth, no network deps, no real artifact required). Source files:
  - `scripts/demo/mockup.html` — pixel-accurate VS Code Dark+ mockup with command-palette, quick-pick, notification, and Simple Browser states all pre-rendered and toggled via `body[data-state="…"]`.
  - `scripts/demo/playwright-report.html` — the fake Playwright HTML report loaded inside the Simple Browser iframe.
  - `scripts/demo/record-video.mjs` — the recorder. Spins up a tiny static server, injects helpers for chapter cards / overlays / state transitions, and renames the autogenerated `.webm` to `media/demo-explorer.webm`.
- **`scripts/record-demo.ps1`** — companion Windows PowerShell script that drives a **real** VS Code instance via `ffmpeg gdigrab` + SendKeys, against a real GitHub PR/run URL. Uses an isolated `--user-data-dir` + `--extensions-dir` profile so it never disturbs your day-to-day VS Code. `-DryRun` flag prints the planned sequence without launching anything. Useful for verifying the mockup matches reality.
- Added `playwright` as a devDependency (used only by the recorder; not bundled in the published `.vsix`).
- Added `scripts/README.md` documenting both recording paths and when to use each.
- `.vscodeignore` now excludes `scripts/` from the published `.vsix` — the recording infrastructure is repo-only.
